...
def build_model(algorithm: str, random_state: int, num_class: int) -> object:
	...
	if algorithm == "SVM (RBF)":
		return SVC(kernel="rbf", probability=True, random_state=random_state, gamma="scale")
...
def render_page_quality(df_quality: pd.DataFrame) -> None:
	...
	if len(corr_cols) >= 2:
		...
		st.markdown("#### Conditional correlation by quality level")
		quality_level = st.selectbox("Filter by quality level", ["All"] + df_quality["quality_label"].unique().tolist())
		
		if quality_level == "All":
			corr_subset = df_quality[corr_cols]
		else:
			corr_subset = df_quality[df_quality["quality_label"] == quality_level][corr_cols]
		
		if len(corr_subset) > 1:
			st.plotly_chart(plot_correlation_heatmap(corr_subset, corr_cols), use_container_width=True)
			st.caption(f"Correlation matrix for quality level: {quality_level}")
	else:
	...
	st.markdown("### Advanced visualization 1: Violin plots (distribution shape)")
	violin_feature = st.selectbox(
		"Feature for violin plot",
		box_feature_options,
		index=box_feature_options.index("sulphates") if "sulphates" in box_feature_options else 0,
		key="violin_selector"
	)
	fig_violin = px.violin(
		...
		box=True,
		points="all",
		title=f"{violin_feature} - violin plot with embedded box plot",
	)
	st.plotly_chart(fig_violin, use_container_width=True)

	st.markdown("### Advanced visualization 2: Parallel coordinates (multivariate)")
	parallel_features = st.multiselect(
		"Select features for parallel coordinates",
		options=[col for col in analytic_cols if col != "quality"],
		default=["alcohol", "sulphates", "fixed acidity", "pH", "residual sugar"][:min(5, len(analytic_cols))],
		key="parallel_selector"
	)
	if len(parallel_features) >= 2:
		parallel_df = df_quality[parallel_features + ["quality"]].copy()
		fig_parallel = px.parallel_coordinates(
			parallel_df,
			color="quality",
			labels={col: col for col in parallel_features + ["quality"]},
			color_continuous_scale=px.colors.diverging.Tealrose,
			title="Parallel coordinates plot - quality patterns",
		)
		st.plotly_chart(fig_parallel, use_container_width=True)
	else:
		st.info("Select at least 2 features for parallel coordinates.")

	st.markdown("### Advanced visualization 3: 2D density contour (relationships)")
	density_x = st.selectbox(
		"X-axis for density",
		analytic_cols,
		index=analytic_cols.index("alcohol") if "alcohol" in analytic_cols else 0,
		key="density_x"
	)
	density_y = st.selectbox(
		"Y-axis for density",
		analytic_cols,
		index=analytic_cols.index("sulphates") if "sulphates" in analytic_cols else 1,
		key="density_y"
	)
	fig_density = px.density_contour(
		...
		title=f"2D density contour: {density_x} vs {density_y}",
	)
	...
	st.markdown("### t-SNE visualization (advanced dimensionality reduction)")
	tsne_perplexity = st.slider("t-SNE perplexity", min_value=5, max_value=50, value=30, step=5)
	with st.spinner("Computing t-SNE embedding..."):
		numeric_cols = [c for c in df_quality.columns if df_quality[c].dtype != "O"]
		numeric_cols = [c for c in numeric_cols if c not in {"quality", "high_quality_flag"}]
		features = df_quality[numeric_cols].fillna(df_quality[numeric_cols].median())
		scaler = StandardScaler()
		scaled = scaler.fit_transform(features)
		...
		fig_tsne = px.scatter(
			...
			color="quality_label",
			hover_data=["quality", "alcohol", "sulphates"],
			title="t-SNE projection (nonlinear manifold)",
			color_discrete_sequence=px.colors.qualitative.Set2,
		)
		st.plotly_chart(fig_tsne, use_container_width=True)

	st.markdown("### DBSCAN clustering (density-based)")
	dbscan_eps = st.slider("DBSCAN eps", min_value=0.1, max_value=2.0, value=0.5, step=0.1)
	dbscan_min_samples = st.slider("DBSCAN min_samples", min_value=2, max_value=20, value=5, step=1)
	with st.spinner("Running DBSCAN..."):
		dbscan = DBSCAN(eps=dbscan_eps, min_samples=dbscan_min_samples)
		dbscan_labels = dbscan.fit_predict(scaled)
		dbscan_df = df_quality.copy()
		dbscan_df["dbscan_cluster"] = dbscan_labels.astype(str)
		dbscan_df["pc1"] = clustered_df["pc1"]
		dbscan_df["pc2"] = clustered_df["pc2"]
		fig_dbscan = px.scatter(
			dbscan_df,
			x="pc1",
			y="pc2",
			color="dbscan_cluster",
			hover_data=["quality", "alcohol"],
		...
		st.caption(f"Clusters found: {n_clusters_found}, Noise points: {n_noise}")
...
def render_pairings_page(df_pairings: pd.DataFrame) -> None:
	...
	st.markdown("### Sunburst chart (hierarchical pairing structure)")
	sunburst_fig = px.sunburst(
		filtered,
		path=["wine_category", "cuisine", "food_category"],
		values="pairing_quality",
		color="pairing_quality",
		color_continuous_scale="RdYlGn",
		title="Hierarchical pairing quality breakdown",
	)
	...
	st.markdown("### Animated scatter - pairing evolution by cuisine")
	if len(filtered) > 0 and "cuisine" in filtered.columns:
		anim_df = filtered.copy()
		anim_df["count"] = 1
		anim_summary = anim_df.groupby(["wine_type", "cuisine"]).agg({
		...
		fig_anim = px.scatter(
			...
			x="wine_type",
			y="pairing_quality",
			animation_frame="cuisine",
			size="count",
			color="wine_type",
			hover_name="wine_type",
			title="Pairing quality by wine type across cuisines",
			range_y=[anim_summary["pairing_quality"].min() - 0.5, anim_summary["pairing_quality"].max() + 0.5],
		)
	...
	st.markdown("### Sankey diagram - pairing flow")
	if len(filtered) >= 10:
		sankey_sample = filtered.head(50)
		
		wine_types = sankey_sample["wine_type"].unique().tolist()
		food_cats = sankey_sample["food_category"].unique().tolist()
		cuisines = sankey_sample["cuisine"].unique().tolist()
		
		all_nodes = wine_types + food_cats + cuisines
		...
		for _, row in sankey_sample.iterrows():
			if row["wine_type"] in node_dict and row["food_category"] in node_dict:
				source_indices.append(node_dict[row["wine_type"]])
				target_indices.append(node_dict[row["food_category"]])
				values.append(row["pairing_quality"])
		...
		fig_sankey.update_layout(title_text="Wine to Food Category Flow", height=600)
...
def render_cross_insights(df_quality: pd.DataFrame, df_pairings: pd.DataFrame) -> None:
	...
	st.markdown("### 3D scatter plot - multivariate exploration")
	scatter_3d_x = st.selectbox("3D X-axis", ["alcohol", "sulphates", "fixed acidity", "pH"], key="3d_x")
	scatter_3d_y = st.selectbox("3D Y-axis", ["sulphates", "alcohol", "citric acid", "residual sugar"], key="3d_y")
	scatter_3d_z = st.selectbox("3D Z-axis", ["fixed acidity", "pH", "density", "chlorides"], key="3d_z")
	
	if all(col in df_quality.columns for col in [scatter_3d_x, scatter_3d_y, scatter_3d_z]):
		fig_3d = px.scatter_3d(
			...
			size="quality",
			hover_data=["quality", "alcohol"],
			title=f"3D visualization: {scatter_3d_x} vs {scatter_3d_y} vs {scatter_3d_z}",
			color_discrete_sequence=px.colors.qualitative.Vivid,
		)
		fig_3d.update_layout(height=700)
		st.plotly_chart(fig_3d, use_container_width=True)

	st.markdown("### Statistical significance tests (ANOVA)")
	from scipy.stats import f_oneway
	
	st.write("Testing if wine features differ significantly across quality labels:")
	anova_results = []
	numeric_features = df_quality.select_dtypes(include=[np.number]).columns
	numeric_features = [f for f in numeric_features if f not in ["quality", "high_quality_flag"]]
	
	for feature in numeric_features[:10]:
		groups = [df_quality[df_quality["quality_label"] == label][feature].dropna() 
		          for label in df_quality["quality_label"].unique()]
	...
	st.markdown("### Radar chart - wine quality profiles")
	selected_features = ["alcohol", "sulphates", "fixed acidity", "residual sugar", "pH"]
	available_features = [f for f in selected_features if f in df_quality.columns]
	
	if available_features:
		quality_profiles = df_quality.groupby("quality_label")[available_features].mean()
		
		fig_radar = go.Figure()
		for quality_label in quality_profiles.index:
			values = quality_profiles.loc[quality_label].values.tolist()
			values += values[:1]
		...
		fig_radar.update_layout(
			polar=dict(radialaxis=dict(visible=True, range=[0, quality_profiles.max().max() * 1.1])),
			showlegend=True,
			title="Wine quality profiles - radar comparison",
			height=600,
		)
